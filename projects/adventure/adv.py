...
# initialize a found map
found_map = {}
found_map[player.currentRoom.id] = {
    x: '?' for x in player.currentRoom.getExits()}

# store an undiscovered set to use for our base condition
undiscovered = set()
...
while undiscovered:
    ...
    # keep going as long as we have unknown exits in current room
    if '?' in found_map[player.currentRoom.id].values():
        next_move = None
    ...
    # execute a breadth first search on the found map looking for the nearest unknown
        ...
        starting_room = player.currentRoom.id
        queue = Queue()
        for direction, room in found_map[starting_room].items():
        ...
        while not queue.empty():
            ...
            if '?' in [room for direction, room in found_map[current[1]].items()]:
